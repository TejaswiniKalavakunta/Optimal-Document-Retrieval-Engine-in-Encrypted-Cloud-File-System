 ...
 int cnt=0;
	...
	try {
		...
		//PreparedStatement ps = con.prepareStatement("SELECT * FROM upload WHERE fname=?  "); /* and uid=? */
			PreparedStatement ps=con.prepareStatement("SELECT * FROM upload INNER JOIN keywords ON upload.encryptedfilename = keywords.encfilename where keywords.keywords=? or upload.skeyownergenerated=? group by upload.fid");	
			ps.setString(1,t1);
			ps.setString(2,t1);
		//ps.setString(2,s11);
